...
float apply_deadband(float out) {
    if (fabsf(out) < MOTOR_DEADBAND) return 0;
    return (out > 0) ? (out - MOTOR_DEADBAND) : (out + MOTOR_DEADBAND);
}

void Task_motor(void) {
		while (1) {
				
				float u = apply_deadband(output);
			
        if (system_state == SYS_CALIBRATING) {
						pwm_set_duty_cycle(0);
						//u = 0;
						continue;
        } 
				
				if (fabsf(u) < 1.0f) {   // smaller threshold now
						pwm_set_duty_cycle(0);
				} else if (u > 0) {
            set_motor_direction(DIR_REVERSE);
            pwm_set_duty_cycle(clamp(u, 0, 100));
        } else {
